...
مرحباً بكم في مساعد قضية الآلي للاستعلام عن الدعاوى القضائية لدى وزارة العدل الكويتية.

*المميزات / Features:*
• تقرير PDF شامل يتضمّن كافة بيانات الدعوى
• جلسات المحكمة والأحكام الصادرة بالتفصيل
• ملخص تحليلي بتقنية الذكاء الاصطناعي
• روابط لإضافة المواعيد إلى التقويم
• الجدول الزمني الكامل للدعوى

*الاستخدام / How to use:*
يرجى إرسال الرقم الآلي للدعوى، وسيتم إعداد تقرير PDF احترافي بشأنها.

*مثال:* \`222486500\`

_يرجى إرسال رقم الدعوى للبدء._
      `;
...
*كيفية الاستخدام:*
1️⃣ أرسل الرقم الآلي للدعوى (9 أرقام)
2️⃣ انتظر 15-30 ثانية
3️⃣ استلام تقرير PDF شامل
...
_Powered by AI — Qadiya_
    ...
    this.bot.on('text', async (ctx) => {
      ...
      if (!/^\d{6,12}$/.test(caseNumber)) {
        await ctx.reply(
          '⚠️ *رقم دعوى غير صحيح*\n\n' +
          'يرجى إرسال الرقم الآلي للدعوى (6-12 رقم).\n' +
          'مثال: `222486500`',
          { parse_mode: 'Markdown' }
        );
        return;
  ...
  async processCaseInquiry(ctx, caseNumber) {
    ...
    try {
      ...
      const statusMsg = await ctx.reply(
        `🔍 *جارٍ البحث عن الدعوى رقم ${caseNumber}...*\n\n` +
      ...
      try {
        await ctx.telegram.editMessageText(
          ...
          null,
          '📄 _جارٍ إعداد التقرير التحليلي..._\n🤖 تحليل البيانات بتقنية الذكاء الاصطناعي...',
          { parse_mode: 'Markdown' }
        );
      } catch (e) {
        // Ignore edit errors
  ...
  buildCaption(result) {
    ...
    let caption = `✅ *تقرير الدعوى رقم ${caseNumber}*\n\n`;
