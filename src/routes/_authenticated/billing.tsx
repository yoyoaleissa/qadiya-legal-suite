...
export const Route = createFileRoute("/_authenticated/billing")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Time tracking, invoicing, and expense logging for Kuwaiti law firms.",
      },
      { property: "og:title", content: "Billing — Qadiya OS" },
      {
...
function statusLabelFn(s: string, tt: (en: string, ar: string) => string) {
  switch (s) {
    case "paid":
      return tt("Paid", "مدفوعة");
    case "sent":
      return tt("Sent", "مُرسلة");
    case "overdue":
      return tt("Overdue", "متأخرة");
    case "cancelled":
      return tt("Cancelled", "ملغاة");
    default:
      return tt("Draft", "مسودة");
  }
}
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {tt(
              ...
              "الفواتير والمدفوعات والأرصدة المستحقة.",
            )}
          </p>
        </div>
        <Button
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="pt-6">
            <EmptyState
              ...
              desc={tt(
                "Create your first invoice to start tracking payments and outstanding balances.",
                "أنشئ أول فاتورة لبدء تتبع المدفوعات والأرصدة المستحقة.",
              )}
              action={
                <Button
                  className="gap-2 bg-navy text-white hover:bg-navy/90 dark:bg-gold dark:text-navy dark:hover:bg-gold/90"
        ...
        <>
          ...
          <div
            ...
            role="tablist"
            aria-label={tt("Invoice filters", "تصفية الفواتير")}
          >
            <FilterCard
            ...
            <FilterCard
              ...
              value={
                <>
                  {totalPaid.toFixed(3)} <span className="text-xs text-muted-foreground">KWD</span>
                </>
              }
            ...
            <FilterCard
              ...
              value={
                <>
                  {overdueCount}{" "}
                  <span className="text-xs text-muted-foreground">{tt("invoices", "فواتير")}</span>
                </>
              }
                  ...
                  {filtered.length === 0 ? (
                    ...
                    filtered.map((inv) => (
                      <InvoiceRow
                        ...
                        onUpdate={() => queryClient.invalidateQueries({ queryKey: ["invoices"] })}
                      />
                    ))
                  )}
                </tbody>
...
function FilterCard({
  ...
  const activeBg =
    accent === "gold" ? "bg-gold/5" : accent === "success" ? "bg-success/5" : "bg-destructive/5";
  return (
    <button
      type="button"
      role="tab"
...
function InvoiceRow({
  ...
  const displayStatus =
    isOverdue && inv.status !== "paid" && inv.status !== "cancelled" ? "overdue" : inv.status;
    ...
    <tr className="hover:bg-accent/30 transition-colors">
      ...
      <td className="px-4 py-3">
        <span className={lang === "ar" ? "font-arabic" : ""}>
          {lang === "ar" ? (inv.client_name_ar ?? inv.client_name) : (inv.client_name ?? "—")}
        </span>
        {inv.case_number && (
          <span className="block text-xs text-muted-foreground">#{inv.case_number}</span>
        )}
      </td>
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{tt("New Invoice", "فاتورة جديدة")}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4 py-2">
          <div>
            <label className="text-xs text-muted-foreground">
              {tt("Amount (KWD)", "المبلغ (د.ك)")}
            </label>
            <Input
              type="number"
              step="0.001"
          ...
          <div>
            ...
            <Textarea
              ...
              rows={2}
              placeholder={tt("e.g. Legal consultation fees", "مثال: أتعاب استشارة قانونية")}
            />
          </div>
          <div>
            <label className="text-xs text-muted-foreground">
        ...
        <DialogFooter>
          ...
          >
            {loading ? (
              ...
              tt("Create Invoice", "إنشاء الفاتورة")
            )}
          </Button>
        </DialogFooter>
      </DialogContent>
