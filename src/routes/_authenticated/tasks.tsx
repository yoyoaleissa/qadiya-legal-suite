...
function statusTone(s: string) {
  switch (s) {
    ...
    case "in_progress": return "bg-navy/10 text-navy dark:bg-gold/15 dark:text-gold";
...
function TasksPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="pt-6">
            <EmptyState
              ...
              desc={tt("Create a task or run a workflow template to get started.", "أنشئ مهمة أو شغّل قالب سير عمل للبدء.")}
            />
          </CardContent>
        </Card>
      ) : (
        <div className="grid gap-4 lg:grid-cols-5">
                  ...
                  <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                    ...
                    {task.assignee && (
                      <span className="text-[11px] text-muted-foreground inline-flex items-center gap-1">
                        ...
                        <span className={lang === "ar" ? "font-arabic" : ""}>{lang === "ar" ? task.assignee_ar ?? task.assignee : task.assignee}</span>
                      </span>
                    )}
                  </div>
                </div>
          ...
          <div className="lg:col-span-2">
            {selected ? (
              <TaskDetail task={selected} onClose={() => setSelectedId(null)} tt={tt} lang={lang} priorityLabel={priorityLabel} statusLabel={statusLabel} />
            ) : (
              <div className="hidden lg:flex flex-col items-center justify-center gap-2 rounded-lg border border-dashed h-full min-h-[240px] text-center p-6">
                ...
                <div className="text-xs text-muted-foreground">{tt("Its description, priority and assignee will appear here.", "سيظهر هنا الوصف والأولوية والمسؤول.")}</div>
              </div>
            )}
          </div>
        </div>
      ...
      {/* Create Task Dialog */}
      <CreateTaskDialog open={showCreate} onClose={() => setShowCreate(false)} tt={tt} lang={lang} onCreated={() => queryClient.invalidateQueries({ queryKey: ["tasks"] })} />

      {/* Workflow Template Dialog */}
      <WorkflowDialog open={showWorkflow} onClose={() => setShowWorkflow(false)} tt={tt} lang={lang} onCreated={() => queryClient.invalidateQueries({ queryKey: ["tasks"] })} />
    </div>
  );
}

/* ─── Create Task Dialog ─── */
function CreateTaskDialog({ open, onClose, tt, lang, onCreated }: { open: boolean; onClose: () => void; tt: (en: string, ar: string) => string; lang: string; onCreated: () => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader><DialogTitle>{tt("Create Task", "إنشاء مهمة")}</DialogTitle></DialogHeader>
        <div className="space-y-4 py-2">
          <div>
            <label className="text-xs text-muted-foreground">{tt("Title (English)", "العنوان (إنجليزي)")}</label>
            <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={tt("e.g. File appeal memorandum", "مثال: إيداع مذكرة الاستئناف")} />
          </div>
          <div>
            <label className="text-xs text-muted-foreground">{tt("Title (Arabic)", "العنوان (عربي)")}</label>
            <Input value={titleAr} onChange={(e) => setTitleAr(e.target.value)} dir="rtl" placeholder="العنوان بالعربي (اختياري)" />
          </div>
          <div>
          ...
          <div>
            ...
            <Input value={assignee} onChange={(e) => setAssignee(e.target.value)} placeholder={tt("e.g. Sara Al-Rashid", "مثال: سارة الرشيد")} />
          </div>
        </div>
        <DialogFooter>
...
/* ─── Workflow Template Dialog ─── */
function WorkflowDialog({ open, onClose, tt, lang, onCreated }: { open: boolean; onClose: () => void; tt: (en: string, ar: string) => string; lang: string; onCreated: () => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader><DialogTitle>{tt("Run Workflow Template", "تشغيل قالب سير عمل")}</DialogTitle></DialogHeader>
        <div className="space-y-4 py-2">
          <p className="text-sm text-muted-foreground">{tt("Select a template to automatically create a series of tasks with pre-set deadlines.", "اختر قالباً لإنشاء سلسلة مهام تلقائياً بمواعيد محددة مسبقاً.")}</p>
          <div>
            <label className="text-xs text-muted-foreground">{tt("Template", "القالب")}</label>
            <Select value={selectedTemplate} onValueChange={setSelectedTemplate}>
              <SelectTrigger><SelectValue placeholder={tt("Choose a template…", "اختر قالباً…")} /></SelectTrigger>
              <SelectContent>
                {(templates ?? []).map((t) => (
                  <SelectItem key={t.id} value={t.id}>{lang === "ar" ? t.name_ar ?? t.name : t.name}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          {activeTemplate && (
            <div className="rounded-lg border bg-muted/30 p-3 space-y-2">
              <div className="text-xs font-medium text-muted-foreground">{tt("Steps that will be created:", "الخطوات التي ستُنشأ:")}</div>
              {activeTemplate.steps.map((step, i) => (
                <div key={i} className="flex items-center gap-2 text-sm">
                  <span className="text-xs text-muted-foreground w-6">{i + 1}.</span>
                  <span className={lang === "ar" ? "font-arabic" : ""}>{lang === "ar" ? step.title_ar : step.title}</span>
                  <span className="ms-auto text-xs text-muted-foreground">+{step.days_offset}d</span>
                </div>
              ))}
            </div>
          )}
          <div>
            <label className="text-xs text-muted-foreground">{tt("Assign all to", "إسناد الكل إلى")}</label>
            <Input value={assignee} onChange={(e) => setAssignee(e.target.value)} placeholder={tt("Optional — leave blank to assign later", "اختياري — اتركه فارغاً للإسناد لاحقاً")} />
          </div>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>{tt("Cancel", "إلغاء")}</Button>
          <Button onClick={handleRun} disabled={!selectedTemplate || loading} className="bg-navy text-white hover:bg-navy/90 dark:bg-gold dark:text-navy">
            {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : tt("Run Template", "تشغيل القالب")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
/* ─── Task Detail Panel ─── */
function TaskDetail({ task, onClose, tt, lang, priorityLabel, statusLabel }: { task: TaskItem; onClose: () => void; tt: (en: string, ar: string) => string; lang: string; priorityLabel: (p: string) => string; statusLabel: (s: string) => string }) {
  const queryClient = useQueryClient();
  ...
  const calendarUrl = task.due_date ? buildGoogleCalendarUrl({
    title: `📋 ${task.title}`,
    date: task.due_date,
  ...
  return (
    <Card className="sticky top-6">
      <CardContent className="pt-6 space-y-5">
        ...
        <div className="flex flex-wrap gap-2">
          <span className={cn("inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium", priorityTone(task.priority))}><Flag className="h-3 w-3" />{priorityLabel(task.priority)}</span>
          <span className={cn("rounded-full px-2.5 py-0.5 text-xs font-medium", statusTone(task.status))}>{statusLabel(task.status)}</span>
        </div>

        {/* Status Change Buttons */}
        <div className="flex flex-wrap gap-2">
          {task.status !== "open" && (
            <Button size="sm" variant="outline" onClick={() => handleStatusChange("open")} disabled={updating}>{tt("Set Open", "فتح")}</Button>
          )}
          {task.status !== "in_progress" && (
            <Button size="sm" variant="outline" onClick={() => handleStatusChange("in_progress")} disabled={updating}>{tt("Start", "بدء")}</Button>
          )}
          {task.status !== "done" && (
            <Button size="sm" variant="outline" className="border-success/50 text-success hover:bg-success/10" onClick={() => handleStatusChange("done")} disabled={updating}>{tt("Complete", "إكمال")}</Button>
          )}
        </div>

        <div>
          <div className="text-xs uppercase tracking-widest text-muted-foreground mb-1">{tt("Description", "الوصف")}</div>
          <p className="text-sm leading-relaxed">{(lang === "ar" ? task.description_ar : task.description) ?? tt("No description.", "لا يوجد وصف.")}</p>
        </div>

        <div className="grid gap-4">
          <DetailRow icon={User} label={tt("Assigned to", "مسند إلى")}>
            {task.assignee ? <span className={lang === "ar" ? "font-arabic" : ""}>{lang === "ar" ? task.assignee_ar ?? task.assignee : task.assignee}</span> : tt("Unassigned", "غير مُسند")}
          </DetailRow>
          ...
          {task.case_number && (
            <DetailRow icon={FileText} label={tt("Related case", "القضية المرتبطة")}>
              <span className="block"><span className={lang === "ar" ? "font-arabic" : ""}>{lang === "ar" ? task.case_title_ar ?? task.case_title : task.case_title}</span></span>
        ...
        {calendarUrl && (
          <a href={calendarUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 rounded-lg border border-dashed p-3 text-sm text-muted-foreground hover:border-gold hover:text-gold transition-colors">
...
function DetailRow({ icon: Icon, label, children }: { icon: typeof User; label: string; children: React.ReactNode }) {
  return (
    <div className="flex items-start gap-3">
      <div className="mt-0.5 h-8 w-8 shrink-0 rounded-md bg-muted flex items-center justify-center text-muted-foreground"><Icon className="h-4 w-4" /></div>
      <div className="min-w-0"><div className="text-xs text-muted-foreground">{label}</div><div className="text-sm font-medium">{children}</div></div>
    </div>
  );
}
