  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 max-w-lg">
        <Input
          placeholder={tt("Enter case number (e.g. 222486500)", "أدخل رقم القضية (مثلاً 222486500)")}
          value={input}
        ...
        <Card>
          <CardContent className="py-12 flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-muted-foreground">
              {tt("Generating report with AI analysis...", "جارٍ إنشاء التقرير مع التحليل الذكي...")}
            </p>
          </CardContent>
        </Card>
      )}
      ...
      {report && !report.found && (
        <Card>
          <CardContent className="py-12 flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-muted-foreground">
              {tt("Case not found in database. Try a different number.", "القضية غير موجودة في قاعدة البيانات. جرب رقماً آخر.")}
            </p>
          </CardContent>
        </Card>
      )}
      ...
      {report && report.found && (
        <div className="space-y-4">
            ...
            <CardContent className="py-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 mb-2">
                <Button
                  ...
                  title={getNextHearing() ? `Next: ${getNextHearing()!.date}` : "No upcoming hearings"}
                >
                ...
                <Button
                  ...
                  title={getAppealDeadline() ? `Deadline: ${getAppealDeadline()}` : "No appeal deadline"}
                >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    navigator.clipboard.writeText(`Case ${report.case_number} — ${lang === "ar" ? report.status_headline_ar : report.status_headline_en}`);
                ...
                {report.deadline && (
                  <Badge variant="destructive" className="flex items-center gap-1 h-9 justify-center text-xs">
                    ⏰ {report.deadline.days_remaining} {tt("days left", "يوم متبقي")}
                  </Badge>
                )}

                {getNextHearing() && (
                  <Badge variant="outline" className="flex items-center gap-1 h-9 justify-center text-xs border-gold/50 text-gold">
              ...
              {showNoteInput && (
                <div className="mt-3 flex gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (noteText.trim()) {
                        alert(tt("Note saved (requires Supabase connection)", "تم حفظ الملاحظة (يتطلب اتصال Supabase)"));
                        setNoteText("");
          ...
          <ReportView report={report} onNew={() => { setReport(null); setInput(""); }} />
        </div>
      )}
